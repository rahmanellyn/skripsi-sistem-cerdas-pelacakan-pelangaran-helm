...
studi ini berfokus pada pengembangan sistem end-to-end. Sistem yang diusulkan
mengintegrasikan deteksi, pelacakan, dan evaluasi kinerja simpang secara
real-time dengan model YOLOv8 dan arsitektur \textit{big data streaming}.
Pendekatan ini melampaui studi terdahulu yang umumnya berhenti pada deteksi
atau pelacakan. Luarannya adalah analisis lalu lintas yang lebih adaptif dan
terukur melalui alur otomatis.
...
beragam. Karakteristik ini membuat pendekatan pengolahan tradisional menjadi
kurang efisien. Karakteristik utama \textit{Big Data} dikenal sebagai "5V"
\cite{marz2015big}:

\begin{enumerate}[a.]
  \item \textit{Volume}: Data yang dihasilkan sangat besar, misalnya aliran video 24/7 dari empat kamera CCTV dan data pelanggaran yang terus bertambah pada \textit{data warehouse}. Kondisi ini membutuhkan infrastruktur penyimpanan dan pemrosesan yang skalabel.
...
\item \textit{Value}: Data perlu menghasilkan nilai guna, misalnya dalam bentuk dasbor analitik untuk mendukung pengambilan keputusan.
\end{enumerate}

Penelitian ini menghadapi tantangan utama pada aspek \textit{volume} dan
\textit{velocity}. Kondisi tersebut mendorong penggunaan
...
Kappa menawarkan latensi rendah dan konsistensi implementasi
\cite{kreps2014questioning}. Dampaknya, pengembangan, pengujian, dan
pemeliharaan sistem analitik menjadi lebih efisien.
...
keandalan tinggi. Dalam arsitektur data \textit{real-time}, Kafka berperan
...
\cite{ayae2017video}, yang menunjukkan bahwa Kafka mampu menjaga kontinuitas
aliran data secara stabil.

Kafka menyimpan data secara arsitektural sebagai rekaman berurutan di dalam
...
dilacak dengan presisi dan diputar ulang (\textit{replay}) saat dibutuhkan.
...
\textit{publish-subscribe} ini menjadikan Kafka sesuai sebagai
...
sehingga analitik \textit{end-to-end} tetap responsif dan konsisten
\cite{ayae2017video}.
...
dua tahap seperti R-CNN, YOLO memformulasikan deteksi sebagai masalah regresi
...
Representasi ini menunjukkan bahwa \(x, y\) menyatakan koordinat pusat kotak
...
ukuran citra masukan. Formulasi ini membantu model menjaga konsistensi
lokalisasi ketika skala objek berubah. Formulasi ini juga mempermudah
...
menyeimbangkan kesalahan prediksi koordinat, ukuran kotak,
...
identitas lintas waktu. Kebutuhan ini mendorong penggunaan pendekatan
...
menjaga kontinuitas identitas objek saat bergerak.

ByteTrack mengatasi keterbatasan asosiasi konvensional dengan tetap
mempertimbangkan deteksi berskor rendah yang kerap muncul akibat
...
meningkatkan stabilitas pelacakan dan menurunkan risiko kehilangan identitas
pada lingkungan dinamis \cite{zhang2022bytetrack_eccv}.

\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{gambar/bytraking_pipeline.png}
...
mendapatkan pasangan paling andal. Tahap kedua meninjau lintasan yang belum
...
Pendekatan ini efektif meningkatkan stabilitas pelacakan pada kondisi
...
dihasilkan secara lebih komprehensif. Data di dalam \textit{data warehouse}
...
jangka panjang, serta perbandingan antarkurun waktu. Dalam praktik terkini,
konsep ini juga diadopsi pada
...
pelanggaran, dan waktu, juga membentuk struktur deskriptif yang stabil untuk
analisis lintas sudut pandang.

Dari perspektif performa, \textit{data mart} menyediakan jalur agregasi yang
...
semantik yang stabil untuk definisi indikator kinerja utama (KPI), sehingga
istilah, rumus, dan konteks analitik tetap konsisten pada seluruh dasbor.
...
berisiko meningkatkan latensi, inkonsistensi hasil, dan kompleksitas
pemeliharaan sistem.

\subsubsection{Star Schema}

\textit{Star schema} merupakan paradigma pemodelan data multidimensi yang
...
Pendekatan \textit{star schema} memiliki beberapa keunggulan metodologis:
\begin{enumerate}[a.]
...
\textit{big data warehouse}. Platform ini terintegrasi dengan berbagai sistem,
...
\textit{Extract--Transform--Load} (ETL) sebelum ditampilkan pada dasbor.
Pendekatan ini mendukung paradigma \textit{real-time analytics}, sehingga
...
Makna metrik dapat dibaca secara sederhana. \textit{True Positive} (TP) adalah
...
Klasifikasi tersebut menjadi dasar kuantitatif untuk menilai keseimbangan
antara kemampuan model mendeteksi objek relevan dan risiko kesalahan deteksi.
Keterkaitan antarkomponen ini penting untuk menafsirkan metrik lanjutan,
...
Evaluasi performa deteksi objek umumnya menggunakan metrik
...
\end{equation}
Nilai AP yang tinggi menunjukkan bahwa model mampu mempertahankan presisi
...
berjalan kontinu dengan jeda serendah mungkin
\cite{kreps2014questioning}. Dua metrik utama yang digunakan adalah
\textit{throughput} dan \textit{latency}, yang bersama-sama merepresentasikan
...
Sebaliknya, \textit{latency} menggambarkan jeda antara data diterima dan hasil
...
\textit{stateful} dan efisiensi komunikasi antarkomponen. Sistem streaming
dinilai optimal jika mampu mempertahankan \textit{throughput} tinggi dengan
\textit{latency} rendah, idealnya pada rentang \textit{sub-second}. Kondisi
ini penting untuk aplikasi visi komputer yang membutuhkan respons cepat,
seperti deteksi pelanggaran lalu lintas \cite{kreps2014questioning}.
