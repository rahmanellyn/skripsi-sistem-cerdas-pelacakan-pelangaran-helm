...
Distribusi pada Tabel~\ref{tab:distribusi-objek} difokuskan pada dua kelas pelanggaran yang menjadi inti penelitian, yaitu \texttt{DRIVER\_HELMET} sebanyak 730 anotasi dan \texttt{DRIVER\_NO\_HELMET} sebanyak 718 anotasi. Selisih antara keduanya hanya 12 anotasi atau setara rasio 1{,}02:1, sehingga dapat dianggap setara. Keseimbangan tersebut menjadi syarat utama agar model tidak cenderung memilih salah satu kategori saat membedakan pengendara berhelm dan tanpa helm. Kelas \texttt{MOTORCYCLE} memuat 921 anotasi atau lebih banyak sekitar 191 hingga 203 anotasi dibandingkan dua kelas pelanggaran. Selisih tersebut wajar karena \texttt{MOTORCYCLE} berperan sebagai kelas konteks; badan kendaraan tampak utuh pada hampir setiap citra, sedangkan area kepala pengendara kerap terhalang sebagian oleh kaca depan atau penumpang sehingga jumlah anotasinya sedikit lebih rendah.

Suatu dataset mulai dikategorikan mengalami \textit{class imbalance} apabila nilai \textit{imbalance ratio} (IR) melebihi 1,5:1\cite{kraiem2021resampling}. Pada penelitian ini, rasio kelas mayoritas (\texttt{MOTORCYCLE}) terhadap kelas minoritas (\texttt{DRIVER\_NO\_HELMET}) sebesar 921:718 atau 1,28:1, masih berada di bawah ambang tersebut. Distribusi ketiga kelas dengan demikian tergolong seimbang sehingga risiko bias model ke kelas mayoritas dapat dikatakan rendah.

Selain distribusi kuantitatif, kualitas dataset juga ditinjau dari posisi kemunculan objek pada bidang pandang kamera. Gambar~\ref{fig:heatmap-per-class} menyajikan \textit{annotation heatmap} dengan gradasi biru hingga merah pekat sebagai indikator intensitas \textit{bounding box} per \textit{grid} citra.
...
Pada Gambar~\ref{fig:heatmap-per-class}, ketiga kelas terkonsentrasi di bagian tengah hingga kanan atas citra. Pola tersebut mengikuti lintasan utama kendaraan roda dua di gerbang masuk sehingga anotasi pada latar pasif, seperti langit, trotoar, dan jalur berlawanan, sangat sedikit. Berdasarkan pola tersebut, ekstraksi \textit{Region of Interest} (ROI) yang menyusur kontur jalan dapat memangkas pemrosesan area latar dan mengarahkan inferensi YOLOv8 pada zona yang relevan. Sebaran \texttt{DRIVER\_NO\_HELMET} sedikit melebar ke arah kiri citra sehingga mengindikasikan kebiasaan pengendara mengambil sisi tertentu saat memasuki gerbang. Penyempitan area komputasi melalui ROI berpotensi menaikkan \textit{precision} sekaligus menekan \textit{false positive} pada inferensi waktu nyata.
...
Strategi augmentasi disusun untuk mereplikasi gangguan visual yang lazim terjadi pada kamera pengawas luar ruang. Visualisasi setiap teknik augmentasi disajikan pada Gambar~\ref{fig:augmentasi-visual}.
...
Pada Gambar~\ref{fig:augmentasi-visual}, setiap transformasi geometris dan fotometrik membidik kondisi gangguan tertentu di lapangan. Rotasi ($-8^\circ$ hingga $+8^\circ$) dan \textit{shear} ($\pm 10^\circ$) pada Gambar~\ref{fig:augmentasi-visual}(a) dan (b) mensimulasikan pergeseran sudut pandang akibat getaran tiang atau hembusan angin. Variasi ini melatih model mengenali fitur spasial kendaraan tanpa bergantung pada orientasi horizon sempurna. Penyesuaian \textit{exposure} ($\pm 10\%$) dan \textit{additive noise} (hingga 1,09\%) pada Gambar~\ref{fig:augmentasi-visual}(c) dan (e) mereplikasi fluktuasi iluminasi harian serta kondisi cuaca. Augmentasi fotometrik semacam ini menekan risiko \textit{overfitting} terhadap intensitas warna tertentu dan mendorong representasi semantik yang lebih kuat \cite{shorten2019survey}. Pada aspek dinamika objek, Gambar~\ref{fig:augmentasi-visual}(d) dan (f) memperagakan \textit{Gaussian blur} (hingga 1,4 piksel) dan \textit{motion blur}. Distorsi fokus dan artefak gerak melatih model menjaga akurasi deteksi lintas \textit{frame} ketika kualitas fitur target menurun sementara.
