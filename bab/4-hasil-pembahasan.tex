...
Bab 1 menanyakan bagaimana model YOLOv8 dirancang, dilatih, dan dievaluasi untuk mendeteksi pengendara sepeda motor berhelm dan tanpa helm pada video CCTV kampus dengan akurasi yang terukur. Sub-bab ini menjawab pertanyaan tersebut dengan urutan kerja yang paralel dengan Tahap 1 dan Tahap 2 metodologi pada Bab 3, yaitu analisis dataset, pra-pemrosesan dan augmentasi, dinamika pelatihan, serta evaluasi kinerja sebelum model diintegrasikan ke \textit{pipeline} pemrosesan video waktu nyata. Pembahasan dibagi menjadi lima bagian. Empat bagian pertama mengikuti urutan kerja pengembangan model, dan bagian terakhir merangkai keempat hasil menjadi jawaban atas Rumusan Masalah ke-1.
% ------------------------------------------------------------
...
% ------------------------------------------------------------
\subsection{Sintesis Jawaban atas Rumusan Masalah Pertama}
\label{subsec:sintesis-rm1}

Empat sub-bab di atas menghasilkan empat bukti yang saling melengkapi. Dataset kustom CCTV gerbang Itera dengan distribusi tiga kelas yang seimbang (\textit{imbalance ratio} 1,28:1) dan augmentasi enam ragam tersusun rapi sebagai dasar pelatihan. Dinamika 300 \textit{epoch} pelatihan memperlihatkan kurva \textit{train loss} dan \textit{val loss} yang turun konsisten dengan jarak wajar, jadi pelatihan tidak \textit{overfit} maupun \textit{underfit}. Pada himpunan validasi, model YOLOv8 mencapai mAP@50 91,4\% dengan \textit{precision} 87,5\% dan \textit{recall} 86,5\% --- ketiga metrik tersebut sejajar dan menandakan model tidak berat sebelah ke satu sisi. Selisih hanya 1,4 poin antara mAP@50 validasi (91,4\%) dan uji (90,0\%) menandakan model masih dapat diandalkan ketika berhadapan dengan citra yang belum pernah dilihat saat pelatihan. Dengan keempat bukti tersebut, pertanyaan Rumusan Masalah ke-1 sudah memperoleh jawabannya, yaitu model YOLOv8 berhasil dirancang, dilatih, dan dievaluasi pada video CCTV kampus dengan kinerja yang terukur lewat mAP@50, \textit{precision}, dan \textit{recall} sehingga modul deteksi sudah cukup matang untuk diintegrasikan ke \textit{pipeline} pemrosesan tepi pada sub-bab berikutnya.
...
Bab 1 menanyakan bagaimana skema \textit{data mart} berbasis \textit{star schema} dan dasbor analitik dirancang untuk mengubah \textit{event stream} pelanggaran helm menjadi instrumen Sistem Pendukung Keputusan (\textit{Decision Support System}) yang memetakan pola spasiotemporal pelanggaran di Itera. Sub-bab ini menjawab pertanyaan tersebut dengan urutan kerja yang paralel dengan Tahap 4 metodologi pada Bab 3, yaitu implementasi \textit{star schema} di PostgreSQL, visualisasi dasbor Grafana, dan analisis wawasan lapangan atas rekaman operasional 10 jam di gerbang utama Itera. Pembahasan dibagi menjadi empat bagian. Tiga bagian pertama mengikuti urutan kerja lapisan hilir, dan bagian terakhir merangkai keempat hasil menjadi jawaban atas Rumusan Masalah ke-3.
% ------------------------------------------------------------
...
% ------------------------------------------------------------
\subsection{Sintesis Jawaban atas Rumusan Masalah Ketiga}
\label{subsec:sintesis-rm3}

Tiga sub-bab di atas menghasilkan empat bukti yang saling melengkapi. Skema bintang dengan tabel fakta \texttt{fact\_violations} dan tiga tabel dimensi (\texttt{dim\_camera}, \texttt{dim\_violation\_type}, \texttt{dim\_time}) terbukti mampu memuat 113 insiden pelanggaran dari rekaman 10 jam operasional di gerbang utama Itera secara persisten dan siap untuk kueri agregasi. Dasbor Grafana memvisualisasikan keempat sudut pandang yang dibutuhkan pengelola: tren agregat harian, distribusi 15 menit, akumulasi jam, dan matriks temporal kepadatan; keempatnya dirakit dari sumber data yang sama tanpa duplikasi. Analisis wawasan lapangan menonjolkan pola spasiotemporal nyata, yaitu distribusi bimodal pagi--sore dengan puncak absolut pukul 14.45--15.15 WIB yang berkorelasi dengan transisi jadwal kelas dan kondisi pasca-hujan. Stabilitas \textit{Avg Confidence} 79,9\% dan \textit{Avg Latency} 49 milidetik selama lonjakan beban memperlihatkan dasbor masih dapat dipercaya saat data mengalir deras. Dengan keempat bukti tersebut, pertanyaan Rumusan Masalah ke-3 sudah memperoleh jawabannya, yaitu skema \textit{data mart} berbasis \textit{star schema} dan dasbor analitik Grafana berhasil mengubah \textit{event stream} pelanggaran helm menjadi instrumen Sistem Pendukung Keputusan yang memetakan pola spasiotemporal pelanggaran di kampus Itera secara terukur dan dapat ditindaklanjuti.
